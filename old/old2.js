...
//let modelStarters = [firstOrder, secondOrder, balancemodels, compoundmodels,compoundmodels,compoundmodels];
let desc = [
  {
...
document.getElementById('button-left').addEventListener('click', () => {
  ...
  document.getElementById('pop-up-content').innerHTML=desc[currentModels].description;
  clear();
...
function checkForInput(){
  ...
  Object.entries(models).forEach(([i, m]) => {
    ...
    if (mouseIsPressed && dist(_x, _y, mouseX, mouseY) < unit * m.shape.diameter / 2) {
  ...
  Object.entries(models).forEach(([i, m]) => {
    ...
    m.flows_out.forEach((flowOut) => {
     if (flowOut.type === "dependent"){
       ...
       let independent = nodeTo.initial*(1-flowOut.value);
       let dependent = flowOut.value * ((v-m.initial)/nodeTo.initial);
       
       models[flowOut.flow_to].value=dependent+independent;
       console.log(nodeTo);
...
Plotly.d3.json('https://raw.githubusercontent.com/plotly/datasets/master/3d-ribbon.json', function (figure) {
  ...
  Plotly.newPlot('chartContent', data, layout);
});
